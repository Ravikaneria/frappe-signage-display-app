...
# ─────────────────────────────────────────────────────────────────────────────
# This runs server-side for every page load of /display  OR  /display/<id>
#
# Frappe www page routing:
#   /display            → this file, path_arg = None
#   /display/ABC123     → this file, path_arg = "ABC123"
#
# The screen_id is passed to the template as `context.screen_id` so the JS
# can send heartbeats and fetch the right signages.
# ─────────────────────────────────────────────────────────────────────────────
...
def get_context(context):
    ...
    # Pass CSRF token for authenticated fetch() calls
    context.csrf_token = frappe.session.csrf_token

    # ── Detect screen_id from URL path ────────────────────────────────────────
    # frappe passes the trailing path segment as frappe.form_dict.path
    # e.g. for /display/ABC123  →  frappe.form_dict.path == "ABC123"
    ...
    # ── Signages for initial SSR (optional — JS also fetches on load) ─────────
