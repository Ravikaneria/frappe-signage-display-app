...
frappe.ui.form.on("Signage", {
    ...
    content_type: function (frm) {
        ...
        // Duration override not meaningful for video (auto-advances on end)
    ...
    youtube_url: function (frm) {
        ...
        if (m) {
            ...
            frappe.show_alert({
                message: "Could not detect YouTube video ID — check the URL",
                indicator: "orange",
            });
        }
    },
    ...
    display_image: function (frm) {
        if (frm.doc.display_image) {
            frappe.show_alert({
                message: "Image will be auto-resized to max 1920×1080 on save",
                indicator: "blue",
            });
        }
    },
